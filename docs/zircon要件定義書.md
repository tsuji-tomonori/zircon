...
「zircon」では、プロジェクト単位のロールベースアクセス制御(RBAC)を基本としつつ、さらにポリシーベースの柔軟な拡張を行うハイブリッド構造を採用します。これにより、プロジェクトごとに異なる運用要件や特殊なステータス承認フローに対応できるように設計します。本章では、ロールの階層構造、ポリシー言語Cedarを用いた詳細権限管理、リソース粒度の制御、特殊操作の権限設定、および監査とコンプライアンス要件について定義します。
